...
/*   Updated: 2025/06/20 15:57:19 by ggoncalv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_env *search_var_in_env(t_shell *shell, char *arg)
{
	...
	if (ft_strchr(arg, '='))
		len = (size_t)(ft_strchr(arg, '=') - arg); //dif de ponteiros p obter o indice
	else
		len = ft_strlen(arg);
	export = shell->env;
	while (export)
	{
		if (export->export && ft_strncmp(export->key, arg, len) == 0 && ft_strlen(export->key) == len)
			return (export);
		export = export->next;
	}
	return (NULL);
...
/**
 * @brief Inserts a new environment variable node into the shell's env list in sorted order.
 *
 * @param shell The shell state.
...
/**
 * @brief Creates a new environment variable node and inserts it into the env list.
 *
 * @param shell The shell state.
